...
import pokemons, { PokemonModel } from "modules/multiply/util/pokemonLocal";
import { getCountVisiblePokemons } from "modules/multiply/util/helpers";
...
const UserPokedex = ({ height = 500, width = 450 }: Props) => {
  const [open, setOpen] = useState(false);
  const [clickedPokemon, setClickedPokemon] = useState<null | PokemonModel>(
    null
  );
  const {
    userSettings: { pokedexIds, countGames },
  } = useUserSettings();
        ...
        {pokedexIds.map((id: number, index: number) => {
          ...
          return (
            <ImageListItem
              key={id.toString()}
              onClick={() =>
                visiblePokemon ? handleClickOpen(id) : handleClickOpenDisabled
              }
            >
